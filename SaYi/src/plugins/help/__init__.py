...
SaYi_info = on_command("关于SaYi", priority=10, block=True)
...
async def handle_function():
    if delay():
        return
    try:
        file_path = 'file:///SaYi/picture/WMLibrary.png'
...
async def handle_function():
    if delay():
        return
    try:
        await SaYi_info.finish("""---{SaYi(publish)功能列表}---   
         
{关于SaYi -查看蚕豆给我写的介绍}

---【世忆图书馆】SaYi998---
世忆图书馆是蚕豆用于存放【七十五个世界】的工作室，近来用于暂留Ai助理、进行测试。
@bot，并在你的问题前面加上冒号，我将回答你的问题。
* example “：你好”
---codeforces---
cf -怕你尴尬随机反馈给你一些废话
绑定cf id -QQ号绑定cf名
