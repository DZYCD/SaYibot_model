...
cf_contest_info = on_command("cf赛事", aliases={"查询cf比赛", "近期cf"}, priority=10, block=True)
...
async def handle_function(args: Message = CommandArg()):
    if delay():
        return
    try:
        res = str(args.extract_plain_text())
        if(len(res)):
            return
...
image_id = 'C:\\Pycharm_projects\\SaYibot\\SaYi\\src\\plugins\\codeforces\\codeforces_user.png'
