...
message_list_poke = ["可爱捏", "有话直说，别戳", "...", "怎么了", "干嘛"]
...
update_info = on_command("更新日志", priority=9, block=True)
job_path = "C:/Pycharm_projects/SaYibot/SaYi/src/plugins/Bot_is_online/update.json"
...
async def _(event: Event):
    ...
    try:
        ...
        msg = [MessageSegment.text("SaYi现在是[ PRMoment.3 ]版本")]
        for i in info:
...
async def _():
    if delay():
        return
    try:
        await night.finish(random.choice(message_list_poke))
    except MatcherException:
        raise
    except Exception as e:
        pass  # do something here